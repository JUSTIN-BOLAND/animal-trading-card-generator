...
function myFunc() {
    ...
    scriptChange.src = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyATqhFoPpyH4zjoRny77ewGCm2JB2ZaaTg&cx=partner-pub-4004792896178081:8406642760&q=' + queryInput + '&siteSearch=https://en.wikipedia.org/wiki/&callback=information';
    document.getElementById("title").innerHTML = queryInput.toUpperCase();
    document.getElementById("image").setAttribute("alt", queryInput);
    let colorBackGround = "background-color:" + document.getElementById("colorPicker").value;
    document.getElementById("shiftColor").setAttribute("style", colorBackGround);
    let colorCardBackGround = "background-color:" + document.getElementById("colorPickerCard").value;
    document.getElementById("shiftCardColor").setAttribute("style", colorCardBackGround);
...
function information(response) {
    ...
    scriptChange.src = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyATqhFoPpyH4zjoRny77ewGCm2JB2ZaaTg&cx=partner-pub-4004792896178081:8406642760&q=' + queryInput + '&exactTerms=interesting+facts&callback=facts';
...
function facts(response) {
    ...
    scriptChange.src = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyATqhFoPpyH4zjoRny77ewGCm2JB2ZaaTg&cx=partner-pub-4004792896178081:8406642760&q=' + queryInput + '&exactTerms=scientific+name&callback=scientificName';
}
...
function scientificName(response) {
    ...
    if (scientificNameArray === "Sorry, there is not any relevant search results to be shown :'(") {
        document.getElementById("scientificNameString").innerHTML = "Scientific Name:" + scientificNameArray;
        const queryInput = document.getElementById('query').value;
        let scriptChange = document.getElementById('lifespan');
        scriptChange.src = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyATqhFoPpyH4zjoRny77ewGCm2JB2ZaaTg&cx=partner-pub-4004792896178081:8406642760&q=' + queryInput + '&exactTerms=lifespan&callback=handlerrr';
        ...
        for (let i = 0; i < scientificNameArray.length; i++) {
            ...
            if (findName !== -1) {
                let nameString = scientificNameArray[i].slice(findName, scientificNameArray[i].length);
                ...
                for (let w = 2; w < arrayName.length; w++) {
                    if (arrayName[w].replace(/[^A-Za-z0-9_]/g, "") !== undefined) {
                        arrayNameFiltered.push(arrayName[w].replace(/[^A-Za-z0-9_]/g, "").toUpperCase());
                    }
                }
        ...
        let scientificSingleFiltered = (scientificSingle.filter(filter => filter !== "")).filter(filter => filter !== undefined);
        let scientificFiltered = ((scientific.filter(filter => filter !== "")).filter(filter => filter !== undefined)).filter(filter => filter !== "undefined undefined");

        //Checking If both arrays have any element

        if (scientificFiltered.length !== 0 || scientificSingleFiltered.length !== 0) {
            ...
            if (callDouble[0] === 1 && callSingle[0] === 1) {
                ...
                document.getElementById("scientificNameString").innerHTML = "<br>" + "Scientific Name:" + "<em>" + " " + scientificSingleFiltered[c] + "</em>";
                ...
                document.getElementById("scientificNameString").innerHTML = "<br>" + "Scientific Name:" + "<em>" + " " + scientificFiltered[callDouble[1]] + "</em>";
                ...
                document.getElementById("scientificNameString").innerHTML = "<br>" + "Scientific Name:" + "<em>" + " " + scientificSingleFiltered[callSingle[1]] + "</em>";
            }
            ...
            const queryInput = document.getElementById('query').value;
            let scriptChange = document.getElementById('lifespan');
            scriptChange.src = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyATqhFoPpyH4zjoRny77ewGCm2JB2ZaaTg&cx=partner-pub-4004792896178081:8406642760&q=' + queryInput + '&exactTerms=lifespan&callback=lifespan';
        }
    }
}
...
function lifespan(response) {
    ...
    if (getLifespan === "Sorry, there is not any relevant search results to be shown :'(") {
        document.getElementById("lifespanString").innerHTML = "<br>" + "Lifespan:" + getLifespan;
        ...
        if (timePeriodArray.length !== 0) {
            ...
            document.getElementById("lifespanString").innerHTML = "<br>" + timePeriodArray[d].charAt(0).toUpperCase() + timePeriodArray[d].slice(1);
...
function searchLifespan(arrString, timeperiod) {
    ...
    for (let i = 0; i < arrString.length; i++) {
        ...
        if (lifespanValue !== -1) {
            let stringTimeperiod = arrString[i].slice(arrString[0], lifespanValue + (timeperiod).length)
